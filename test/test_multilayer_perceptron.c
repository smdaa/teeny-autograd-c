...
static void test_forward_multilayer_perceptron(void **state) {
  ...
  snprintf(path, sizeof(path), "%s/test_forward_multilayer_perceptron/x.txt",
           dataDir);
  ndarray *x = read_ndarray(path);

  snprintf(path, sizeof(path), "%s/test_forward_multilayer_perceptron/y.txt",
           dataDir);
  ndarray *y = read_ndarray(path);

  snprintf(path, sizeof(path),
           "%s/test_forward_multilayer_perceptron/weights0.txt", dataDir);
  ndarray *weights0 = read_ndarray(path);

  snprintf(path, sizeof(path),
           "%s/test_forward_multilayer_perceptron/weights1.txt", dataDir);
  ndarray *weights1 = read_ndarray(path);

  snprintf(path, sizeof(path),
           "%s/test_forward_multilayer_perceptron/weights2.txt", dataDir);
  ndarray *weights2 = read_ndarray(path);

  snprintf(path, sizeof(path),
           "%s/test_forward_multilayer_perceptron/bias0.txt", dataDir);
  ndarray *bias0 = read_ndarray(path);

  snprintf(path, sizeof(path),
           "%s/test_forward_multilayer_perceptron/bias1.txt", dataDir);
  ndarray *bias1 = read_ndarray(path);

  snprintf(path, sizeof(path),
           "%s/test_forward_multilayer_perceptron/bias2.txt", dataDir);
  ndarray *bias2 = read_ndarray(path);

  snprintf(path, sizeof(path), "%s/test_forward_multilayer_perceptron/z.txt",
           dataDir);
  ndarray *z = read_ndarray(path);

  snprintf(path, sizeof(path),
           "%s/test_forward_multilayer_perceptron/weights0_grad.txt", dataDir);
  ndarray *weights0_grad = read_ndarray(path);

  snprintf(path, sizeof(path),
           "%s/test_forward_multilayer_perceptron/weights1_grad.txt", dataDir);
  ndarray *weights1_grad = read_ndarray(path);

  snprintf(path, sizeof(path),
           "%s/test_forward_multilayer_perceptron/weights2_grad.txt", dataDir);
  ndarray *weights2_grad = read_ndarray(path);

  snprintf(path, sizeof(path),
           "%s/test_forward_multilayer_perceptron/bias0_grad.txt", dataDir);
  ndarray *bias0_grad = read_ndarray(path);

  snprintf(path, sizeof(path),
           "%s/test_forward_multilayer_perceptron/bias1_grad.txt", dataDir);
  ndarray *bias1_grad = read_ndarray(path);

  snprintf(path, sizeof(path),
           "%s/test_forward_multilayer_perceptron/bias2_grad.txt", dataDir);
...
int main(void) {
  ...
  if (dataDir == NULL) {
    fprintf(stderr, "Error: TEST_MULTILAYER_PERCEPTRON_DATA_DIR environment "
                    "variable not set.\n");
    return 1;
  }
