...
NDARRAY_TYPE sample_uniform() { return ((NDARRAY_TYPE)rand() / RAND_MAX); }
...
NDARRAY_TYPE sample_truncated_normal(NDARRAY_TYPE mean, NDARRAY_TYPE std,
                                     NDARRAY_TYPE lo, NDARRAY_TYPE hi) {
  NDARRAY_TYPE sample;
  do {
...
void matmul_2d_ndarray_helper(const NDARRAY_TYPE *a, const NDARRAY_TYPE *b,
                              NDARRAY_TYPE *c, int p, int q, int r) {
  for (int i = 0; i < p; i++) {
    for (int k = 0; k < q; k++) {
